...
import {
  ...
  healPlayer,
  JACKPOT_POOLS,
} from "../../src/lib/yamada-minecraft.js";
import {
  biomes,
  travelRequirements,
  pickaxes,
  ...
  SMELT_RECIPES,
  CRAFT_RECIPES,
} from "../../src/lib/yamada-minecraft-data.js";
...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "yamada-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = YAMADA_CORE_CONFIG.saluran?.id || "120363400911374213@newsletter";
  const sName = YAMADA_CORE_CONFIG.saluran?.name || YAMADA_CORE_CONFIG.bot?.name || "yamada-ai";
  return {
