...
import { generateWAMessage, generateWAMessageFromContent, jidNormalizedUser } from '@itsliaaa/baileys';
...
try {
    if (fs.existsSync('./assets/images/yamada2.jpg')) {
        cachedThumb = fs.readFileSync('./assets/images/yamada2.jpg')
    }
} catch (e) {}
...
function getContextInfo(title = '📢 ᴊᴘᴍ ᴀʟʙᴜᴍ', body = 'Album Broadcast') {
    ...
    const saluranName = config.saluran?.name || config.bot?.name || 'yamada-ai'
        ...
        for (const groupId of groupIds) {
            ...
            try {
                if (mediaList.length >= 2) {
                    ...
                    const contextInfo = getContextInfo('📢 ᴊᴘᴍ ᴀʟʙᴜᴍ', config.bot?.name || 'yamada')
