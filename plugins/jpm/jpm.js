...
import { fetchGroupsSafe } from "../../src/lib/yamada-jpm-helper.js";
import {
  getAutoJpmConfig,
  setAutoJpmConfig,
...
} from "../../src/lib/yamada-auto-jpm.js";
import { getMimeType, getExtension } from "../../src/lib/yamada-utils.js";
...
try {
  cachedThumb = getAssetBuffer("yamada2");
} catch {}
...
async function sendInteractiveMessage(
  ...
  const saluranName = YAMADA_CORE_CONFIG.saluran?.name || botName;
...
async function handleJpmUpdateWithContent(m, sock, db, input) {
  ...
  let version = YAMADA_CORE_CONFIG.bot?.version || "v1.0";
  let changelog = input;
  if (input.includes("|")) {
...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${YAMADA_CORE_CONFIG.bot?.name || "yamada-ai"} AutoJPM`,
    body,
    footer: `${YAMADA_CORE_CONFIG.bot?.name || "yamada-ai"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${YAMADA_CORE_CONFIG.bot?.name || "yamada-ai"} Delay`,
      body,
      footer: `${YAMADA_CORE_CONFIG.bot?.name || "yamada-ai"} JPM System`,
      buttons,
    });
  }
...
async function handleBlacklist(m, sock, db, settingKey, label) {
  ...
  if (!m.text || m.text.trim().startsWith("_")) {
    ...
    let listText =
      ...
      `Berikut *${groups.length} grup* yang diikuti bot *${YAMADA_CORE_CONFIG.bot?.name}*\n` +
