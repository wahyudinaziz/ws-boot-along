...
function getCtx() {
  const saluranId = YAMADA_CORE_CONFIG.saluran?.id || "";
  const saluranName = YAMADA_CORE_CONFIG.saluran?.name || YAMADA_CORE_CONFIG.bot?.name || "";
...
function startGiveawayChecker(sock, db) {
  new CronJob(
    ...
    async () => {
      try {
        const { getDatabase } = await import("../../src/lib/yamada-database.js");
        const currentDb = getDatabase();
        const { getSocket } = await import("../../src/handler.js");
