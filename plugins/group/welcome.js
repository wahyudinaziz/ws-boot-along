...
import { createWideDiscordCard } from "../../src/lib/yamada-welcome-card.js";
import { resolveAnyLidToJid } from "../../src/lib/yamada-lid.js";
...
function resolvePlaceholders(
  ...
  return template
    ...
    .replace(/{bot}/gi, YAMADA_CORE_CONFIG.bot?.name || "yamada")
...
async function sendWelcomeMessage(sock, groupJid, participant, groupMeta) {
  try {
    ...
    if (welcomeType === 2) {
      ...
      // WhatsApp interactive cards cannot receive a plain { url } in imageMessage.
      // Prepare/upload the remote profile picture first, then insert the generated
      // imageMessage protobuf into the carousel header.
      let carouselImageMessage;
      try {
        const media = await prepareWAMessageMedia(
          { image: { url: ppUrl } },
          { upload: sock.waUploadToServer },
        );
        carouselImageMessage = media?.imageMessage;
      } catch (mediaError) {
        console.error("Welcome V2 media preparation failed:", mediaError);
        carouselImageMessage = null;
      }

      const cardHeader = carouselImageMessage
        ? { title: "Welcome", hasMediaAttachment: true }
        : { title: "Welcome", hasMediaAttachment: false };

      const card = {
        header: carouselImageMessage
          ? { imageMessage: carouselImageMessage, hasMediaAttachment: true }
          : { title: "👋 Welcome", hasMediaAttachment: false },
        body: { text: cardBody },
      ...
      await sock.sendMessage(groupJid, {
        video: getAssetBuffer("yamada-mp4") || { url: "https://files.catbox.moe/k28dhp.mp4" },
        gifPlayback: true,
        caption: text,
        contextInfo: {
          mentionedJid: [realParticipant],
        ...
        viewOnceMessage: {
          message: {
            ...
            interactiveMessage: {
              ...
              footer: {
                text: YAMADA_CORE_CONFIG.bot?.name || "yamada-ai"
              },
              contextInfo: {
                mentionedJid: [realParticipant],
                isForwarded: true,
