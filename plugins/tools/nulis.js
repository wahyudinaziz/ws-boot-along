...
const fontUrl = getAssetBuffer("yamada-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("yamada-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("yamada-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("yamada-kertas");
