...
async function handler(m, { sock }) {
    ...
    if (sub === 'on') {
        ...
        return m.reply(
            `🦋 *ANTI PM AKTIF* 🦋\n\n` +
            `> Yamada: "Siap darling~ Sekarang cuma ${allowedNumbers.length} orang yang bisa chat ke aku!\n\n` +
    ...
    if (sub === 'off') {
        ...
        return m.reply(
            `❌ *ANTI PM NONAKTIF* ❌\n\n` +
            `> Yamada: "Hmph~ Yaudah, sekarang semua orang bisa chat ke aku. Tapi jangan aneh-aneh ya darling~ 🦋"`
        )
    }
    ...
    return m.reply(
        ...
        `💫 *Yamada:* "Yang lain jangan coba-coba ya darling~ 🎐"`
    )
}
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            if (currentCount === 0) {
                ...
                await sock.sendMessage(sender, {
                    text: `🚨 *YAMADA - PERINGATAN TERAKHIR* 🚨\n\n` +
                ...
                await sock.sendMessage(sender, {
                    text: `💀 *YAMADA - KAMU DIBLOKIR ASLI!* 💀\n\n` +
                        ...
                        try {
                            ...
                            await sock.sendMessage(allowedJid, {
                                text: `💀 *ANTI PM - BLOKIR ASLI* 💀\n\n` +
                                      ...
                                      `🦋 *Yamada:* "Beres darling! Gak bakal ganggu lagi~ 🗿🔥"`
                            })
                        } catch (err) {}
                    }
