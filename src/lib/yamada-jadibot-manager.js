...
import { addJadibotOwner } from "./yamada-jadibot-database.js";
import { extendSocket } from "./yamada-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("yamada2")) {
            thumbnail = getAssetBuffer("yamada2");
          }
        } catch { }
