...
import { addExifToWebp, DEFAULT_METADATA } from "./yamada-exif.js";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./yamada-profile-picture.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "yamada-ai";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = YAMADA_CORE_CONFIG.bot?.name || "yamada-ai";
    if (source) {
      let data = source;
