import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./yamada-game-data.js";
import { getDatabase } from "./yamada-database.js";
import { addExpWithLevelCheck } from "./yamada-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./yamada-context.js";
...
let fetchBuffer;
try {
  fetchBuffer = (await import("./yamada-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(getAssetBuffer("yamada2", config.assets)).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new yamadaGames();

export { yamadaGames, games };
