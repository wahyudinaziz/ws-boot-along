...
function checkPermission(m, pluginConfig) {
  ...
  if (pluginConfig.isOwner && !m.isOwner && !hasAccess) {
    return {
      allowed: false,
      reason: config.messages?.ownerOnly || "👑 Hmph! Fitur ini khusus Owner. Jangan membantah Yamada. 💙",
    };
  }

  if (pluginConfig.isPartner && !m.isPartner && !m.isOwner && !hasAccess) {
    return { allowed: false, reason: "💙 Fitur ini khusus Partner. Jangan memaksa Yamada, ya.", };
  }
  ...
  // Akses premium 100% percuma untuk semua pengguna - lewati pengecekan premium
  if (false && isPremiumFeature && !m.isPremium && !m.isOwner && !m.isPartner && !hasAccess) {
    return {
      allowed: false,
      reason: config.messages?.premiumOnly || "💎 Fitur Premium ini belum terbuka untukmu. Jangan membuat Yamada menunggu. 💙",
    };
  }
  ...
  if (pluginConfig.isGroup && !m.isGroup) {
    return {
      allowed: false,
      reason: config.messages?.groupOnly || "👥 Hmph! Command ini hanya untuk grup. 💙",
    };
  }
  ...
  if (pluginConfig.isPrivate && m.isGroup) {
    return {
      allowed: false,
      reason: config.messages?.privateOnly || "📱 Hmph! Datang ke chat pribadi Yamada dulu. 💙",
    };
  }
  ...
  if (
    ...
    return {
      allowed: false,
      reason: config.messages?.adminOnly || "👑 Hmph! Kamu harus menjadi Admin grup dulu. 💙",
    };
  }
  ...
  if (pluginConfig.isBotAdmin && m.isGroup && !m.isBotAdmin) {
    return {
      allowed: false,
      reason:
        config.messages?.botAdminOnly || "🤖 Jadikan Yamada Admin dulu. Yamada harus bisa melindungimu dengan bebas. 💙",
    };
  }
    ...
    if (group) {
      if (pluginConfig.category === "game" && group.game === false) {
        if (!m.isAdmin && !m.isOwner && !hasAccess) {
          return {
            allowed: false,
            reason: "🎮 *Hmph!* Fitur Game sedang dimatikan oleh Admin. Jangan memaksa Yamada, ya. 💙",
          };
        }
      }
      if (pluginConfig.category === "rpg" && group.rpg === false) {
        if (!m.isAdmin && !m.isOwner && !hasAccess) {
          return {
            allowed: false,
            reason: "⚔️ *Hmph!* Fitur RPG sedang dimatikan oleh Admin. Kalau ingin Yamada membukanya, minta Admin mengaktifkannya lagi. 💙",
          };
        }
      }
    }
