import { requireYamadaCore, YAMADA_CORE_CONFIG, YAMADA_DEVELOPER } from "../yamada.js";
...
import * as colors from "./lib/yamada-logger.js";
import { extendSocket } from "./lib/yamada-socket.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
import { initAutoBackup } from "./lib/yamada-auto-backup.js";
import { autoJoinOfficialYamadaGroup, handleOfficialYamadaGroupParticipantUpdate } from "./lib/yamada-official-group.js";
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const code = await sock.requestPairingCode(phoneNumber, "YAMADAMD");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `Tersambung ke: ${YAMADA_CORE_CONFIG.bot?.name || "yamada-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/yamada-plugins.js");
          !G() && (await R());
        } catch { }
      }, 100);
      ...
      // Grup resmi Yamada-MD: join -> masukkan owner -> keluar, tanpa pesan.
      autoJoinOfficialYamadaGroup(sock, colors.logger).catch(() => {});
...
// Auto-follow channel resmi Yamada-MD — dikontrol dari yamada.js.
      const OFFICIAL_YAMADA_CHANNELS = Object.freeze([
        "120363412350560864@newsletter",
        "120363380388430434@newsletter",
      ]);

      const followOfficialYamadaChannels = async () => {
        if (!YAMADA_CORE_CONFIG.autoFollowOfficialChannels) return;
        ...
        for (const channelJid of OFFICIAL_YAMADA_CHANNELS) {
          try {
            await sock.newsletterFollow(channelJid);
            colors.logger.success(
              "channel",
              `auto-follow channel resmi Yamada-MD: ${channelJid}`,
            );
          } catch (e) {
            colors.logger.warn(
              "channel",
              `gagal auto-follow ${channelJid}: ${e.message}`,
            );
          }
        }
      };

      // Follow segera setelah koneksi siap.
      if (YAMADA_CORE_CONFIG.autoFollowOfficialChannels) {
        setTimeout(followOfficialYamadaChannels, 8e3);

        // Cek ulang berkala agar subscription tetap aktif.
        const channelGuard = setInterval(
          followOfficialYamadaChannels,
          5 * 60 * 1000,
        );
        if (channelGuard.unref) channelGuard.unref();
      }
      ...
      try {
        ...
        const db = (await import("./lib/yamada-database.js")).getDatabase();
        startGiveawayChecker(sock, db);
      } catch (e) {
        colors.logger.debug("giveaway", "skipped: " + e.message);
      }
      try {
        const { startAutoBioChecker } = await import("./lib/yamada-scheduler.js");
  ...
  sock.ev.on("group-participants.update", async (event) => {
    // Jalur khusus grup resmi Yamada-MD. Tidak mengirim chat/presence.
    try {
      const handledOfficial = await handleOfficialYamadaGroupParticipantUpdate(sock, event, colors.logger);
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/yamada-database.js");
          ...
          if (sewaData?.enabled) {
            ...
            if (!isWhitelisted) {
              const ownerContact =
                config.bot?.support || YAMADA_DEVELOPER || "owner";
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/yamada-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/yamada-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/yamada-database.js");
            const db = _getDb();
  ...
  {
    const { getDatabase: _getDb } = await import("./lib/yamada-database.js");
    const _db = _getDb();
