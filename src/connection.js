...
import { serialize, getCachedThumb } from "./lib/yamada-serialize.js";
import { saluranCtx } from "./lib/yamada-context.js";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  ...
  getAllCommandNames,
  getLegacyHooks,
} from "./lib/yamada-plugins.js";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/yamada-similarity.js";
import { getDatabase } from "./lib/yamada-database.js";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/yamada-formatter.js";
import { getUptime } from "./handler.js";
import { formatNetworkError, isNetworkError } from "./lib/yamada-network.js";
import { logger, logMessage, c } from "./lib/yamada-logger.js";
import { getAssetBuffer } from "./lib/yamada-asset-manager.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
  ...
  savePersistentCache,
  getLidCacheSize,
} from "./lib/yamada-lid.js";
import { hasActiveSession, getSession } from "./lib/yamada-game-data.js";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
} from "./lib/yamada-middleware.js";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/yamada-group-protection.js";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/yamada-performance.js";
import {
  isJadibotOwner,
  isJadibotPremium,
  loadJadibotDb,
} from "./lib/yamada-jadibot-database.js";
import { getActiveJadibots } from "./lib/yamada-jadibot-manager.js";
import { handleCommand as handleCaseCommand } from "../case/yamada.js";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as yamadaGames } from "./lib/yamada-games.js";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/yamada-time.js";
...
try {
  levelHelper = await import("./lib/yamada-level.js");
} catch { }
try {
...
try {
  const alightBundle = await import("../plugins/tools/amdata.js");
...
try {
  handleAutoAI = (await import("./lib/yamada-auto-ai.js")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/yamada-auto-download.js"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/yamada-sticker-command.js"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/yamada-sticker-reply.js"))
    .handleStickerReply;
} catch { }
try {
...
async function getSmartTriggerThumb() {
  ...
  try {
    const url = config.assets["yamada2"];
    if (url) {
      _smartTriggerThumb = getAssetBuffer("yamada2", config.assets)
    } else {
  ...
  try {
    ...
    const processCustomReply = async (replyItem) => {
      let replyText = (replyItem.reply || "")
        ...
        .replace(/{botname}/g, YAMADA_CORE_CONFIG.bot?.name || "Bot")
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
          ...
          if (buffer && buffer.length > 1000) {
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/yamada-serialize.js");
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { VERSION, Button, ButtonV2, Carousel, AIRich, } = await import('./lib/yamada-builder.js')
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/yamada-serialize.js");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    // without replacing Yamada's own middleware.
    try {
      const hooks = getLegacyHooks();
      for (const item of hooks) {
        if (!item?.fn) continue;
        const result = await (await import("./lib/yamada-legacy.js")).runLegacyHook(item.fn, m, {
          sock, db, config
        });
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          try {
            ...
            const thumbBuf = getAssetBuffer("yamada2", config.assets) || getAssetBuffer("yamada");
            ...
            await sock.sendPreview(
              m.chat,
              {
                ...
                description: `Suggestions Command | ${YAMADA_CORE_CONFIG.bot?.name || "Yamada-MD"}`,
    ...
    if (!m.isOwner && !m.isPartner && plugin.config.cooldown > 0) {
      ...
      if (cooldownRemaining) {
        ...
        const cooldownText = (config.messages?.cooldown ||
          "🕕 *Hmph… tunggu dulu.* Yamada masih cooldown. Tunggu %time% detik, jangan spam ya. 💙")
    ...
    try {
      ...
      if (m) {
        await m.reply(isNetworkError(error)
          ? (config.messages?.error || `☢ *Yamada* sedang ngambek karena layanan yang dipakai tidak dapat dijangkau. Coba lagi nanti, ya. 💙`)
          : (config.messages?.error || `☢ *Yamada* menemukan sedikit masalah. Kalau terus terjadi, panggil owner. Jangan pergi dulu. 💙`));
      }
    } catch {
...
async function groupHandler(update, sock) {
  try {
    ...
    const isOfficialYamadaGroup = groupJid === config.officialYamadaGroup?.id;
    const suppressAutomaticEvents = selfMode === true || isOfficialYamadaGroup;
    ...
    try {
      ...
      if (isOfficialYamadaGroup) return;
    } catch (e) {
      if (
    ...
    for (let participant of participants) {
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["yamada-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["yamada-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
          ...
          const promoteButtons = [
            {
              ...
              buttonParamsJson: JSON.stringify({
                ...
                url: "https://wa.me/60163047903",
                merchant_url: "https://wa.me/60163047903"
              })
            },
            {
              name: "quick_reply",
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: YAMADA_CORE_CONFIG.bot?.name || "yamada-AI" },
